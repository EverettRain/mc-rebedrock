// world::rendersAsCubeItem —— 三条物品渲染面（掉落物 / 第一人称手持 / HUD 背包图标）
// 共用的「按立方体几何画，还是按挤出的 2.5D 图标画」单一判断。
//
// 这个测试存在的理由是一个真实发生过的静默 bug：这三处曾各自手写
// `model == Cube || model == Chest || ...`，三份口径互不相同——
// RN-4a 引入 DirectionalCube（侦测器）时补进了手持与图标两处，漏了掉落物那处，
// 于是同一个侦测器拿在手里是立方体、放进背包是立方体、掉在地上却是一张扁平贴图。
// 三处合一之后，这里把口径本身钉死，任何新增的 BlockModel 都必须在这里表态。
//
// 同时 include BlockAtlasLayout.hpp：它那组 static_assert 把图集固定特殊区
// 各段的起始层号表述成「上一段起点 + 上一段层数」，编译进这个目标就等于把
// 层号表也一并校验了（改了某段层数却忘了顺移后面的起点，这个测试先编译不过）。
...
// 台阶在内是因为它走同一条立方体路径，只是 Y 向减半——调用方读 isSlab 决定。
...
// 回归点：侦测器是 DirectionalCube，掉落物路径当年就是漏了它。
...
// 明确留在集合外的那些：十字植物、火把、作物、楼梯/门这类异形方块。
// HUD 图标另有一层 isShapedBlockModel 近似把楼梯之类也画成 3D 方块，
// 那是图标独有的，不属于本判断——若哪天把它推广到掉落物与手持物，
// 该改的是那一层，而不是悄悄把它们塞进这个集合。
...
// 与 isFullCube 的关系：那个谓词回答的是「填不填满自己那一格」（遮挡、面稳固），
// 因此排除了箱子和台阶——但两者拿在手里都画成盒子。所以是真包含，不是等价。
