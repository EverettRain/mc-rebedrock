// ui/WidgetLabels.hpp —— 每个 WidgetId 的标签来源分类表。
//
// 覆盖性（每个 id 恰好归一类）已经由该头文件里的 static_assert 在编译期保证，
// 把这个头 include 进本测试就等于在又一个编译单元里跑了一遍那道检查。
// 这里补的是 static_assert 表达不了、或者表达出来读不懂的那部分：
// 表里有没有重复行、有没有空键、以及分类是否与它声称的语义一致。
//
...
// 表行不得重复：同一个 id 出现两次，改其中一行就会静默失效——查找命中的永远是第一行。
...
// 后缀是数据而不是特例分支：vanilla 的 selectWorld.experimental 只有
// "Experimental"，省略号由这一列拼上。目前只有它一行用到。
...
// 分类互斥，且运行期那一档确实不在静态表里——否则渲染器算出来的实时文本
// 会被静态表先一步拦截返回，症状是「分辨率按钮永远显示同一个数字」。
...
// 循环选项的标签只能来自 OptionCycle 的表行：它们的文本和点击时的步进读同一行，
