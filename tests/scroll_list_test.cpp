...
void testScreenRowWidths() {
    ...
    CHECK(mc::ui::languageScrollList(layout).rowWidth == mc::ui::kLanguageRowWidth);
    CHECK(mc::ui::keyBindsScrollList(layout).rowWidth == mc::ui::kKeyBindsRowWidth);
    CHECK(mc::ui::worldScrollList(layout).rowWidth == mc::ui::kWorldSelectionRowWidth);
    // 三张列表都铺满逻辑画布的宽度，行在里面居中
    CHECK(mc::ui::languageScrollList(layout).width == 427);
    CHECK(mc::ui::keyBindsScrollList(layout).width == 427);
}
