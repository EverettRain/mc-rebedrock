...
int main() {
    ...
    // A full extract from the published snapshot interpolates feet and stride.
    {
        ...
        snapshot.previousStride = 0.0F;
        snapshot.stride = 2.0F;
        ...
        assert(std::abs(state.walkStride - 1.0F) < 0.0001F);
