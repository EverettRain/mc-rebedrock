...
[[nodiscard]] constexpr BlockInteractionDecision decideBlockInteraction(
    ...
    case world::ContainerType::Anvil:
        return {BlockInteraction::OpenAnvil, InteractionResult::successWithoutItem()};
...
enum class ItemUseAction : std::uint8_t {
    ...
    // distinct so the ignite path can charge FlintAndSteel's durability (one
    // point) rather than consuming a held stack the way PlaceBlock does.
    PlaceFire,
};
