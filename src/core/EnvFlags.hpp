...
//
//   1. 环境变量在运行期不会变，而读它的位置常常在热路径上——`queueStreamBatch`
//      曾在**世界写锁持有期间**每批次调两次 `getenv("MC_REBEDROCK_SMOKE_TEST")`。
//   2. getenv 本身不是线程安全的：POSIX 下它与 setenv 并发是未定义行为，而本项目
//      的区块流送、光照与音频都在自己的线程上跑。把求值收进函数级 static，
//      整个进程就只在第一次调用时读一次，之后是一个纯读的 bool。
//
// 范式取自 core/FrameTrace.hpp 的 traceEnabled() 与 world/ChunkStreamingTrace.hpp
// 的 chunkTraceEnabled()——它们早就是这么写的；这里补上散落在渲染器各处、
// 每次用都现读一遍的其余开关。
