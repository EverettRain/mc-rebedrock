...
enum class PageId {
    ...
    FontSettings,
    // UI-11 / A5：26.1 `WarningScreen` / `SafetyScreen` 那块全屏提示。
    // vanilla 用它挡在首次进入多人游戏之前；本作用它挡在**首次进入高级图形设置**
    // 之前——那一屏里的每一项都能把帧时间打下来一个数量级。
    // 「不再显示」勾上并按 Proceed 后写进 `GameOptions::skipAdvancedGraphicsWarning`。
    AdvancedGraphicsNotice,
