...
ReBedrock 是一个使用 C++20 和 Vulkan 从头开发的体素沙盒客户端。项目以
...
- **运行架构**：渲染与 20 TPS 模拟线程解耦，通过世界读写段、实体渲染快照、事件
  队列和按区块 Tick 调度器交换状态，避免渲染直接遍历正在变化的模拟数据。
