...
#
# RN-55：--near-shadow 8|16|24 固定近段级联那个框的档位（默认 8）。它是每帧读的
# 选项，从前**没被钉**，出图跑哪一档全看那台机器的 options.properties——而它正是
# 这条线上最关键的可变量（用户报的光斑只在 24 档出现）。档位进输出目录名。
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --near-shadow) SHADOW_ARGS+=("$1" "$2"); shift 2 ;;
